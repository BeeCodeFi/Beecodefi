...
export interface InterviewQuestionsTemplateProps {
  title: string;
  topic: string;
...
export default function InterviewQuestionsTemplate({
  ...
  questions,
  theme,
}: InterviewQuestionsTemplateProps) {
  ...
  const { loading, toggleRevision, clearAllRevisions, isMarked, count } = useRevisions(topic);
  const { isRead, markAsRead, readCount } = useInterviewProgress(topic);
  const { sessionFormatted, totalFormatted } = useStudySession(topic);
  const { getNote, saveNote, hasNote, noteCount, isAuthenticated } = useInterviewNotes(topic);
  ...
  const formatAnswer = (answer: string) => {
    ...
    formatted = formatted.replace(/`([^`]+)`/g, `<code class="${theme.codeBg} ${theme.codeText} px-1.5 py-0.5 rounded font-mono text-sm">$1</code>`);
    formatted = formatted.replace(/\*\*(.*?)\*\*/g, `<strong class="${theme.boldText} font-bold">$1</strong>`);
  ...
  const intermediateQs = questions.filter(q => q.difficulty === 'intermediate');
      ...
      <AnimatePresence>
        {showFlashcards && (
          <FlashcardMode
            ...
            accentColor={`${theme.primaryFrom} ${theme.primaryTo}`}
          />
        )}
      </AnimatePresence>

      {/* Header */}
      <div className={`bg-gradient-to-r ${theme.primaryFrom} ${theme.primaryTo} text-white py-16`}>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-2.5 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 ${theme.focusRing} focus:border-transparent transition-all`}
                />
                {searchTerm && (
                  <button
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className={`px-4 py-2.5 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 ${theme.focusRing} focus:border-transparent cursor-pointer`}
                >
            ...
            >
              <div className="w-full px-6 py-5 flex items-start justify-between gap-4">
                <button
                  ...
                  className={`flex-1 flex items-center gap-4 transition-all text-left group -m-5 p-5 rounded-xl ${theme.hoverBgRow}`}
                >
                  <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-md transition-all bg-gradient-to-br ${
                    isRead(question.id)
                      ? 'from-green-500 to-emerald-500'
                      : `${theme.primaryFrom} ${theme.primaryTo}`
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className={`text-lg font-bold text-gray-900 dark:text-white transition-colors ${theme.hoverText}`}>
                      {question.question}
                    </h3>
                  </div>
              ...
              <AnimatePresence>
                {expandedId === question.id && (
                  <motion.div
                    ...
                    className="overflow-hidden"
                  >
                    <div className={`px-6 py-6 border-t-2 ${theme.primaryBorder} bg-gradient-to-br ${theme.answerBg}`}>
                      <div 
                      ...
                      <div className={`flex flex-wrap gap-2 mt-6 pt-4 border-t ${theme.primaryBorder}`}>
                        {question.tags.map((tag) => (
                          <span
                            key={tag}
                            className={`px-3 py-1 ${theme.primaryBg} ${theme.primaryText} rounded-full text-xs font-medium border ${theme.primaryBorder}`}
                          >
                            #{tag}
                          </span>
                        ))}
