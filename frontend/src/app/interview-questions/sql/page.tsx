...
export default function SQLInterviewQuestionsPage() {
  ...
  const { loading, toggleRevision, clearAllRevisions, isMarked, count } = useRevisions('sql');
  const { isRead, markAsRead, readCount } = useInterviewProgress('sql');
  const { sessionFormatted, totalFormatted } = useStudySession('sql');
  const { getNote, saveNote, hasNote, noteCount, isAuthenticated } = useInterviewNotes('sql');

  const filteredQuestions = sqlInterviewQuestions.filter((q) => {
  ...
  const formatAnswer = (answer: string) => {
    ...
    formatted = formatted.replace(/`([^`]+)`/g, '<code class="!bg-purple-100 dark:!bg-purple-900/30 !text-purple-700 dark:!text-purple-400 px-1.5 py-0.5 rounded font-mono text-sm">$1</code>');
    formatted = formatted.replace(/\*\*(.*?)\*\*/g, '<strong class="text-purple-700 dark:text-purple-300 font-bold">$1</strong>');
  ...
  // Calculate stats
  const beginnerQs = sqlInterviewQuestions.filter(q => q.difficulty === 'beginner');
  const intermediateQs = sqlInterviewQuestions.filter(q => q.difficulty === 'intermediate');
  const advancedQs = sqlInterviewQuestions.filter(q => q.difficulty === 'advanced');

  const stats = {
    total: sqlInterviewQuestions.length,
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1">
              <h1 className="text-4xl font-extrabold">SQL Interview Questions</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="relative">
              ...
              <select
                ...
                className="pl-10 pr-8 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent dark:bg-gray-800 dark:text-white appearance-none cursor-pointer"
              >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          ...
                          {isRead(question.id) && (
                            <span className="inline-flex items-center px-2 py-1 rounded text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400">
                              ✓ Read
                            </span>
                          )}
                        </div>
